...
// Constructor
//
RoboClaw::RoboClaw(const char *serialPortName, uint32_t tout)
{
	timeout = tout;
	

	//copy serial port name to local cstring
	strncpy(serialName, serialPortName, CSTRINGLEN);
...
{
	char openRetVal = serial.openDevice(serialName, (unsigned int)speed);
	if (openRetVal == 1)	//if it equals 1, its sucess
	{
		return;
	}


	printf("ERROR WITH OPENING DEVICE:\n", openRetVal);
	...
	{
	case -1:
		printf("serialib returned -1: device not found\n");
		break;

	case -2:
		printf("serialib returned -2: error while opening the device\n");
		break;

	case -3:
		printf("serialib returned -3: error while getting port parameters\n");
		break;

	case -4:
		printf("serialib returned -4: Speed(Bauds) not recognized\n");
		break;

	case -5:
		printf("serialib returned -5: error while writing port parameters\n");
		break;

	case -6:
		printf("serialib returned -6: error while writing timeout parameters\n");
		break;

	case -7:
		printf("serialib returned -7: Databits not recognized\n");
		break;

	case -8:
		printf("serialib returned -8: Stopbits not recognized\n");
		break;

	case -9:
		printf("serialib returned -9: Parity not recognized\n");
		break;

	default:
		printf("serialib returned %d:[unknown return val]\n");
		break;
...
{
	char returnChar;
	serial.readChar(&returnChar, 1);	//read char without waiting
...
{
	char returnChar;
	serial.readChar(&returnChar, timeout);   //read char with waiting
